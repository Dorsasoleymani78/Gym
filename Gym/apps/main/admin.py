...
class periodAdmin(admin.ModelAdmin):
    list_display=('periodName','salon','major','typeOfPeriod', 'salary','Capacity','startDate_at','timestarted_at','timeended_at' ,
                  'click_me')
    list_filter=('Capacity','timestarted_at','timeended_at')
    search_fields=('majorName','salon')
    # get_list_select_related=('major')
 
    
    def _get_Period_major(self,obj):
        try:
           return obj.major.majorName
        except:
            pass
     
    def  majorName(self,obj):
    
       majors=obj.objects.prefetch_related('major')
       for item in majors:
    ...
    def formfield_for_foreignkey(self, db_field, request, **kwargs):
        if db_field.name == "coach"  :
            ...
            kwargs["queryset"] = Coach.objects.filter(majorName__majorName=self._get_Period_major)
